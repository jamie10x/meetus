...
- Mini App native chrome (`TelegramChrome`, `useTelegramBackButton`, the `MainButton` wiring in `RsvpSection`) all guard on `getTelegramWebApp()` returning non-null and no-op otherwise — every call site must keep that guard, since these components render for plain-browser visitors too. Don't wire Telegram's `colorScheme` into Tailwind's own `prefers-color-scheme` dark mode — that was a deliberate scope cut (see architecture.md) to avoid regressing browser users for a Mini-App-only cosmetic win.
