...
- RSVP capacity is enforced inside a transaction with `SELECT ... FOR UPDATE` on the event row (`rsvp.Repository.Join`, and `Cancel` for the waitlist-promotion path below). Don't add RSVP writes that bypass it.
- A full event **waitlists** a join instead of rejecting it (`status = 'waitlisted'`, no ticket issued). Canceling a `going` RSVP promotes the oldest `waitlisted` row to `going` in the same transaction (`rsvp.Repository.Cancel` → `promoteNextWaitlisted`) and notifies them via `rsvp.PromotionNotifier` (satisfied by `*tgbot.Announcer`) in a background goroutine — same `context.Background()` reasoning as auto-announce. `rsvp` must never import `tgbot` (the reverse already does); the notifier interface is declared in `rsvp` itself, same pattern as `channel.Announcer`.
...
- A recurring series has **no separate table** — `events.series_id` is just the first occurrence's own event ID, shared by every event `CreateSeries` inserts in that transaction. Each occurrence is otherwise a fully independent event (own status, own edits); nothing cascades across a series.
- `organizers.is_verified` is denormalized onto `eventSelect`/`trendingSelect`/`nearbySelect` (`o.is_verified`) so the verified badge renders on event cards without an extra request — add it to any new event-listing query too, or verified organizers will silently lose the badge on that path.
- **The Explore map's container height must be an inline `style`, not a Tailwind class** (`EventMap.tsx`) — a `h-[520px]`-style arbitrary class was tried first and rendered as `2px` in this project's build for reasons not fully root-caused. Leaflet reads the container size once at init; if it's `0`, no tiles load and no tile requests even fire. Don't revert to a Tailwind height class here without re-verifying live.
- **`proxy.ts`'s matcher must exclude `icon`/`apple-icon`/`pwa-icon`** — Next's generated icon routes have no file extension in their URL, so the existing `.*\..*` exclusion doesn't catch them, and next-intl's middleware would otherwise 307-redirect them to `/uz/icon` like an ordinary page, breaking the favicon, PWA install icon, and manifest icons all at once.
- The PWA service worker (`public/sw.js`) cache-firsts static assets but is **network-first for page navigations and `/api/me/tickets`** — event listings and RSVP counts change too often for cache-first to be safe there. It also means stale cached JS can survive `npm run dev` restarts in a browser tab that already registered it; see the dev-mode callout in development.md before assuming an edit isn't showing up because of a build problem.
- **Calendar downloads use a Blob object URL (`downloadIcs` in `lib/ics.ts`), never `<a href="data:...">`.** Telegram's in-app Mini App webview navigates to `data:` URLs instead of downloading them even with the `download` attribute set — verified live in this exact webview context, not a theoretical concern.
- JSON-LD embedded in event pages goes through `stringifyJsonLd` (`lib/eventSchema.ts`), never a bare `JSON.stringify` — event titles/descriptions are organizer-controlled text, and unescaped `<` could break out of the `<script>` tag.
